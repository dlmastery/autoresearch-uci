# Autoresearch checkpoint — after Exp180 DISCARD (1h remains Exp167 residual MLP; FT 6/50 Pre-LN val recipe Exp178; t+6 recipe Exp76)
...
- Best FT val: Exp178 Pre-LN dropout 0.1 **22.140** / test **20.674**
...
- Exp179 dropout=0: val 22.200 worse, test 20.509 better. Exp180 dropout=0.2: val 22.568 / test 21.486 worse. Recipe stays Exp178.
...
- **NEW:** weekend typical |need|<=10 n=1435 RMSE **8.10** vs persist **5.19** (2.94% of SSE; need **+0.49** pred_d **+0.92**). Weekday typical 6.73 vs persist 5.03. Weekend calm hours are the extra identity leak.
- **Exp180 DISCARD** FT Pre-LN dropout=0.2. Val **22.568** missed 21.972 and Exp178 22.140. Test **21.486**. Weekend typical 8.10→**7.62** vs Exp178 8.18 (slice moved, beat MLP on that slice) but typical 7.14→**8.18** and dirty-stable 11.99→**17.03**. January 31.22→**36.83**. Stronger dropout damped weekend calm and over-regularized January/dirty-stable.

## This fire
- **Exp180 DISCARD** FT Pre-LN dropout=0.2 (isolated cycle 6/50). 1h champion remains Exp167. FT **6/50**. FT val recipe remains Exp178. Dropout {0, 0.2} closed vs 0.1.
...
## Process
LightGBM **50/50 complete**. CatBoost **50/50 complete**. MLP **50/50 complete**. FT-Transformer **6/50**. Isolation holds. 1h champion is **Exp167 residual MLP**. FT val recipe is Exp178 Pre-LN dropout 0.1. t+6 recipe remains Exp76.

## Next pasteable
Isolate **FT-Transformer** (6/50) from the Exp178 Pre-LN recipe (norm_first=true, dropout 0.1, n_layers=3, batch 32). Next try **batch_size=64**. Do not retry dropout 0/0.15/0.2/0.3. Do not revert Post-LN. Do not mix MLP HPs. 1h champion remains Exp167 until composite beats −21.972.
