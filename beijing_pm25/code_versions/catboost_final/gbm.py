...
class CatBoostBackbone(Backbone):
    ...
    def build(self, config: dict[str, Any], input_shape: tuple[int, ...], n_outputs: int) -> None:
        ...
        if config.get("grow_policy") is not None:
        ...
        if config.get("min_data_in_leaf") is not None:
            params["min_data_in_leaf"] = int(config["min_data_in_leaf"])
        if config.get("model_size_reg") is not None:
            params["model_size_reg"] = float(config["model_size_reg"])
        if config.get("bootstrap_type") is not None:
            params["bootstrap_type"] = str(config["bootstrap_type"])
            if str(config["bootstrap_type"]).lower() != "bayesian":
                params.pop("bagging_temperature", None)
        if config.get("subsample") is not None:
            params["subsample"] = float(config["subsample"])
        if config.get("border_count") is not None:
