...
22 DISCARDs. Notable near-miss: LightGBM Exp25 test **20.905** — DISCARD because val 22.711 lost to the then-champion.

Depth 8/5/3, lr 0.05/0.005, weather lags, raw hour, L1/L2, CatBoost defaults, `is_heating`: no composite gain.
...
1. LightGBM Ke 2017 GOSS (boosting_type=goss) — first unused paper algorithm on the champion backbone
2. DART (Rashmi & Gilad-Bachrach 2015)
3. Onset/collapse features only after paper knobs, or t+6 on the same timestamps
